 ...
 */
export default function UnitMap({ lat, lng, label, zoom = 16, heightClass = "h-80" }) {
  const position = [parseFloat(lat), parseFloat(lng)];

  if (Number.isNaN(position[0]) || Number.isNaN(position[1])) {
    return (
      <div className={`w-full ${heightClass} rounded-3xl bg-gray-100 flex items-center justify-center text-gray-400 text-sm`}>
