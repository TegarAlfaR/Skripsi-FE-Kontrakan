...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // supaya bisa dibaca oleh Navbar
      Cookies.set("token", response.data.token, COOKIE_OPTIONS);
      Cookies.set("payload", JSON.stringify(response.data.payload), COOKIE_OPTIONS);
  ...
  return (
    <div className="min-h-screen bg-[#F6F4EE] flex items-center justify-center px-4 font-body">
      <DesignTokens />
      ...
      <div className="w-full max-w-md bg-white rounded-3xl border border-[#E4E0D6] shadow-xl p-8">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#2F5D50] hover:bg-[#24463C] transition-all duration-300 text-white py-3 rounded-xl font-semibold flex justify-center items-center gap-3 disabled:bg-[#B8B2A3] disabled:cursor-not-allowed focus-ring"
          >
            {loading ? (
              <>
                <LoadingSpinner />
